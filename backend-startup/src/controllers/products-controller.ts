...
// =======================================================
 export const updateProduct = async (req: Request, res: Response): Promise<void> => {
    try {
       ...
       const { id, quantity, supplierId, ...updateData }: Partial<Product & {quantity?: number}> = req.body;
...
// =======================================================
// POST: Ajustar Estoque do Produto (/api/products/adjust-stock)
// =======================================================

export const adjustStock = async (req: Request, res: Response): Promise<void> => {
  try {
    const { productId, quantity, reason } = req.body; // Pega os dados do body
    
    // --- Validações ---
    const adjustmentQuantity = parseInt(quantity, 10);
    const id = parseInt(productId, 10);

    if (isNaN(id) || isNaN(adjustmentQuantity)) {
      res.status(400).json({ error: 'Dados inválidos', message: 'ProductId e Quantity devem ser números.' });
      return;
    }
    
    // Para esta tela, só permitimos entradas (positivas)
    if (adjustmentQuantity <= 0) {
      res.status(400).json({ error: 'Valor inválido', message: 'A quantidade para reabastecer deve ser maior que zero.' });
      return;
    }
    
    // Verifica se o produto existe
    const productExists = await productRepository.exist({ where: { id: id } });
    if (!productExists) {
      res.status(404).json({ error: 'Produto não encontrado', message: `Produto ID ${id} não existe.` });
      return;
    }
    // --- Fim Validações ---

    // Chama o StockService para fazer a mágica
    // Usamos MANUAL_ADJUSTMENT (ou crie 'PURCHASE' no Enum se preferir)
    await stockService.addMovement(
      id,
      adjustmentQuantity, // Quantidade POSITIVA para entrada
      MovementType.MANUAL_ADJUSTMENT 
    );
    
    // Busca o produto atualizado para retornar o novo estoque
    const updatedProduct = await productRepository.findOneBy({ id: id });

    res.status(200).json({
      message: `Estoque do produto ID ${id} reabastecido com ${adjustmentQuantity} unidades.`,
      data: updatedProduct // Retorna o produto com a 'quantity' atualizada
    });

  } catch (error: any) {
    console.error(`Erro ao ajustar estoque:`, error);
    res.status(500).json({ error: 'Erro interno do servidor', message: 'Não foi possível ajustar o estoque.' });
  }
};